"""
====================================================================================================
Modified Cell Interaction Calculation for Single-Column Categorical Variables - Grouped by Hypoxia Class
====================================================================================================
Summary:
This script performs the following steps:
  ...
  3. For a specified maximum distance, calculates an interaction index for each target cell variable
     using the categorical variable column, GROUPED BY BOTH IMAGE ID AND HYPOXIA CLASS
  ...
  5. Saves the final aggregated DataFrame with interaction indices to a CSV file
====================================================================================================
"""
...
logging.info("Starting modified cell interaction calculation with hypoxia class grouping...")
...
# ----------------------------------------------------------------------
# Step 3: Define target variables to analyze
# ----------------------------------------------------------------------
logging.info("Identifying target cell types for interaction calculation...")

# Option 1: Extract all unique cell types
# Uncomment the next line if you want to use all unique cell types
# target_cell_types = sorted(reduced_df['Classification'].unique().tolist())
...
logging.info(f"Selected {len(target_cell_types)} target cell types to analyze")
...
# Identify the reference cell type
reference_type = 'M1 microglia'  # Change this to match your reference cell identifier

# Calculate interaction indices for each target cell type
for target_type in target_cell_types:
    # Skip calculation for reference cells interacting with themselves
    if target_type == reference_type:
        continue
        
    logging.info(f"Calculating interaction index for {target_type}...")
    ...
    logging.info(f"Completed calculation for {target_type}.")

# ----------------------------------------------------------------------
# Step 5: Add the Image_HypoxiaClass identifier to the final DataFrame
# ----------------------------------------------------------------------
# (Already included when we initialized interaction_indices_df)
...
# ----------------------------------------------------------------------
# Step 6: Save the final DataFrame with interaction indices
# ----------------------------------------------------------------------
final_df = interaction_indices_df

output_csv_path = "/lustre/compbio/projects/Multiplex_ihc_image_analysis/hypoxia/intensities/interactions/cell_interactions_by_hypoxia_class_M1microglia_10.csv"
final_df.to_csv(output_csv_path, index=False)
logging.info(f"Final DataFrame saved to CSV at: {output_csv_path}")

# ----------------------------------------------------------------------
# Step 7: Generate summary statistics by hypoxia class
# ----------------------------------------------------------------------
...
if summary_columns:
    ...
    summary_csv_path = "/lustre/compbio/projects/Multiplex_ihc_image_analysis/hypoxia/intensities/interactions/interaction_summary_by_hypoxia_M1microglia_10.csv"
    ...
    logging.info("Summary of interactions by hypoxia class:")
...
logging.info(f"Calculation complete. Generated interaction indices for {len(target_cell_types)-1} cell types.")
